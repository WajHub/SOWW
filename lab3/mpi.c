...
int main(int argc, char **argv)
{
  ...
  if (myrank == 0)
  {
    ...
    // INITAL SENDING - first distribute some ranges to all slaves
    ...
    while (range[0] <= b)
    {
      ...
      // Kopiujemy żądania odbioru do tablicy dla Waitany
      for (int j = 0; j < proccount - 1; j++)
        requests[j] = handlers[j].recv_req;

      // Czekamy na dowolny wynik
      MPI_Waitany(proccount - 1, requests, &completed_idx, &status);

      int target_slave = completed_idx + 1;
      result += handlers[completed_idx].result; // Dodajemy odebrany wynik [cite: 126]

      // Upewniamy się, że poprzedni Isend (zapas) do tego Slave'a wyszedł z bufora [cite: 133]
      MPI_Wait(&handlers[completed_idx].send_req, MPI_STATUS_IGNORE);

      // Przygotowujemy i wysyłamy kolejny zapas [cite: 135, 144]
      range[1] = range[0] + RANGESIZE - 1;
      if (range[1] > b)
        range[1] = b;

      handlers[completed_idx].range[0] = range[0];
      handlers[completed_idx].range[1] = range[1];

      MPI_Isend(handlers[completed_idx].range, 2, MPI_DOUBLE, target_slave, DATA, MPI_COMM_WORLD, &handlers[completed_idx].send_req);

      // Odświeżamy Irecv, by czekać na wynik z tego, co Slave liczy TERAZ [cite: 148]
      MPI_Irecv(&handlers[completed_idx].result, 1, MPI_DOUBLE, target_slave, RESULT, MPI_COMM_WORLD, &handlers[completed_idx].recv_req);
    ...
    // ... po pętli while (range[0] <= b) ...

    // 1. Musimy odebrać wyniki dla wszystkich otwartych żądań Irecv
    for (i = 0; i < proccount - 1; i++)
    {
    ...
    // 2. Musimy odebrać wyniki za te "zapasowe" zadania, które Slave'y liczą TERAZ
    // Każdy Slave dostał jeszcze jedno zadanie przez Isend, za które NIE wystawiliśmy jeszcze Irecv w pętli while
    for (i = 1; i < proccount; i++)
    {
    ...
    // 3. Dopiero teraz zamykamy Slave'y
    for (i = 1; i < proccount; i++)
    {
      MPI_Send(NULL, 0, MPI_DOUBLE, i, FINISH, MPI_COMM_WORLD);
    }
  ...
  {
    double next_range[2];
    double results[2]; // Bufor na dwa wyniki
    int res_idx = 0;
    ...
    while (status.MPI_TAG != FINISH)
    {
      ...
      // Wysyłamy z bezpiecznego bufora results[res_idx]
      MPI_Isend(&results[res_idx], 1, MPI_DOUBLE, 0, RESULT, MPI_COMM_WORLD, &send_req);

      res_idx = 1 - res_idx; // Przełączamy bufor (0 -> 1, 1 -> 0)

      MPI_Wait(&recv_req, &status);
      range[0] = next_range[0];
      range[1] = next_range[1];
    }
    // Na koniec upewnij się, że ostatni Isend wyszedł
    if (send_req != MPI_REQUEST_NULL)
      MPI_Wait(&send_req, MPI_STATUS_IGNORE);
  }
